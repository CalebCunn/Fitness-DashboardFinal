...
async function request(table,options){if(!URL||!KEY)throw new Error('Cloud connection is not configured in this build.');const controller=new AbortController(),timer=setTimeout(()=>controller.abort(),12000);try{const response=await fetch(`${URL}/rest/v1/${table}?user_id=eq.caleb`,{headers,...options,signal:controller.signal});if(!response.ok)throw new Error(`Cloud request failed (HTTP ${response.status}).`);return await response.json();}finally{clearTimeout(timer);}}
