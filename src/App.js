...
const C = {
  ...
  blue:"#60a5fa", orange:"#fb923c", text:"#f1f5f9", sub:"#94a3b8", muted:"#3d4f63",
...
const Input = ({ value, onChange, style={} }) => (
  <input value={value} onChange={e=>onChange(e.target.value)} style={{ background:C.surface, border:`1px solid ${C.teal}40`, borderRadius:6, padding:"5px 8px", color:C.text, fontSize:11, outline:"none", fontFamily:"Inter", width:"100%", ...style }} />
);
...
// ─── OVERVIEW ────────────────────────────────────────────────────────────────
function Overview({ stats, activities, whoopData, whoopOk, onConnectWhoop, bestEfforts, gear }) {
  ...
  const paceTrend = vol.map(w => {
    ...
    const avgPace = weekRuns.length ? weekRuns.reduce((s,r) => s+(r.average_speed||0),0)/weekRuns.length : 0;
    return { week: w.week, pace: avgPace ? parseFloat((1000/avgPace/60).toFixed(2)) : null, km: w.km };
  }).filter(w => w.pace);
  ...
  const loadChart = vol.slice(-8).map(w => {
    ...
    const load = weekRuns.reduce((s,r) => s+(r.suffer_score||0),0);
    return { week: w.week, load, km: w.km };
  });
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      ...
      <Card>
        ...
        <div style={{ height:6, background:C.border, borderRadius:3 }}>
          <div style={{ width:`${progress}%`, height:"100%", background:`linear-gradient(90deg,${C.teal},${C.amber})`, borderRadius:3, transition:"width .3s" }} />
        </div>
      ...
      {paceTrend.length > 2 && (
        <Card>
          <Label color={C.amber}>Avg Pace Trend (min/km — lower is faster)</Label>
          <ResponsiveContainer width="100%" height={120}>
            <LineChart data={paceTrend}>
              ...
              <Tooltip content={<CT/>}/>
              <Line type="monotone" dataKey="pace" name="Pace" stroke={C.amber} strokeWidth={2} dot={{fill:C.amber,r:2}} connectNulls/>
            </LineChart>
          </ResponsiveContainer>
        </Card>
      )}
      ...
      {bestEfforts && Object.values(bestEfforts).some(Boolean) && (
        <Card>
          <Label color={C.purple}>Live PBs from Strava</Label>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8 }}>
...
function Running({ activities, stats, gear }) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      ...
      {/* Live shoe mileage from Strava */}
      {gear && gear.length > 0 && (
        <Card>
          <Label color={C.purple}>Shoe Mileage — Live from Strava</Label>
          <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
            {gear.map((s,i) => {
              const pct = Math.min(100, Math.round(((s.distance||0)/1000) / 800 * 100));
              return (
                <div key={i} style={{ background:C.surface, borderRadius:9, padding:"10px 13px" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:5 }}>
                    <div>
                      ...
                      <div style={{ fontSize:9, color:C.sub, marginTop:1 }}>{s.brand_name||""}</div>
                    </div>
                    <div style={{ fontFamily:C.mono, fontSize:12, fontWeight:700, color:C.purple }}>{((s.distance||0)/1000).toFixed(0)}km</div>
                  </div>
...
function Gym({ activities, userPrefs, onSavePrefs }) {
  ...
  const saveLifts = () => {
    onSavePrefs({ ...userPrefs, lifts: editLifts });
    setEditing(false);
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      <Card>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <EditBtn editing={editing} onToggle={() => { if(editing) saveLifts(); else { setEditLifts(lifts); setEditing(true); }}}/>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
          {(editing ? editLifts : lifts).map((l,i) => (
            <div key={i} style={{ background:C.surface, borderRadius:9, padding:"10px 13px", display:"flex", justifyContent:"space-between", alignItems:"center", gap:10 }}>
              {editing ? (
                <>
                  <Input value={l.name} onChange={v => setEditLifts(prev => prev.map((x,j)=>j===i?{...x,name:v}:x))} style={{ flex:1 }}/>
                  <Input value={l.weight} onChange={v => setEditLifts(prev => prev.map((x,j)=>j===i?{...x,weight:v}:x))} style={{ width:100 }}/>
                  <Input value={`${l.sets}x${l.reps}`} onChange={v => { const [s,r]=(v.split("x")||["3","10"]); setEditLifts(prev => prev.map((x,j)=>j===i?{...x,sets:parseInt(s)||3,reps:parseInt(r)||10}:x));}} style={{ width:60 }}/>
                  <button onClick={()=>setEditLifts(prev=>prev.filter((_,j)=>j!==i))} style={{ background:"transparent", border:"none", color:C.pink, cursor:"pointer", fontSize:16 }}>×</button>
                </>
              ) : (
                <>
          ...
          {editing && <button onClick={()=>setEditLifts(prev=>[...prev,{name:"New Exercise",weight:"0kg",sets:3,reps:10}])} style={{ background:"transparent", border:`1px dashed ${C.border}`, borderRadius:9, padding:"9px", color:C.muted, cursor:"pointer", fontSize:11 }}>+ Add exercise</button>}
        </div>
      </Card>
...
function Recovery({ whoopData, whoopOk, onConnectWhoop }) {
  if (!whoopOk) return (
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      <Card style={{ textAlign:"center", padding:"40px 20px" }}>
        ...
        <div style={{ fontSize:11, color:C.sub, marginBottom:20, lineHeight:1.7, maxWidth:260, margin:"0 auto 20px" }}>Live recovery, HRV, sleep and strain from your Whoop 5.0.</div>
  ...
  const hrvChart = recs.slice(0,14).reverse().map(r => ({
    day: new Date(r.created_at).toLocaleDateString("en-GB",{day:"numeric",month:"short"}),
    hrv: Math.round(r.score?.hrv_rmssd_milli||0),
    rhr: Math.round(r.score?.resting_heart_rate||0),
  }));

  const recChart = recs.slice(0,14).reverse().map(r => ({
    day: new Date(r.created_at).toLocaleDateString("en-GB",{day:"numeric",month:"short"}),
    score: Math.round(r.score?.recovery_score||0),
  }));

  const sleepChart = sleeps.slice(0,14).reverse().map(s => ({
    day: new Date(s.start).toLocaleDateString("en-GB",{day:"numeric",month:"short"}),
    hours: s.score?.stage_summary?.total_in_bed_time_milli ? parseFloat((s.score.stage_summary.total_in_bed_time_milli/3600000).toFixed(1)) : 0,
    score: Math.round(s.score?.sleep_performance_percentage||0),
  }));
            ...
            {cycles.slice(0,7).map((c,i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:10, background:C.surface, borderRadius:7, padding:"8px 11px" }}>
                ...
                <div style={{ flex:1, height:3, background:C.border, borderRadius:2 }}>
                  <div style={{ width:`${Math.min((c.score?.strain||0)/21*100,100)}%`, height:"100%", background:C.amber, borderRadius:2 }}/>
                </div>
    ...
    <Card style={{ opacity:isRest?0.6:1 }}>
      ...
      {expanded && (
        <div style={{ marginTop:12, paddingTop:12, borderTop:`1px solid ${C.border}` }}>
          ...
          {!isRest&&s.dist!=="0km"&&(
            <div style={{ fontSize:10, color:C.muted, textAlign:"center", padding:"6px 0", borderTop:`1px solid ${C.border}` }}>
              Coros API integration coming soon
            </div>
          )}
        </div>
      )}
...
function TrainingPlan({ onChat, externalPlan, onAddToCoros }) {
  ...
  useEffect(() => {
    loadTrainingPlan().then(p => { if(p) setPlan(p); setPlanLoaded(true); });
  }, []);
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      {!plan ? (
        <Card style={{ textAlign:"center", padding:"32px 20px" }}>
          ...
          <div style={{ fontSize:11, color:C.sub, marginBottom:20, lineHeight:1.7, maxWidth:260, margin:"0 auto 20px" }}>Ask Claude in the Chat tab to build your Berlin block and it'll appear here automatically.</div>
        ...
        <>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {plan.sessions.map((s,i) => <SessionCard key={i} session={s} typeC={typeC} onAddToCoros={onAddToCoros}/>)}
          </div>
        </>
      )}
    </div>
...
function Races({ userPrefs, onSavePrefs }) {
  ...
  const save = () => {
    onSavePrefs({ ...userPrefs, races: editRaces, sponsorship: editSponsorship });
    setEditing(false);
  };
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:12, paddingBottom:20 }}>
      ...
      <Card>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12 }}>
          ...
          <EditBtn editing={editing} onToggle={()=>{ if(editing) save(); else { setEditRaces(races); setEditSponsorship(sponsorship); setEditing(true); }}}/>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
              ...
              {editing ? (
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                  <div>
                    ...
                    <div style={{ fontSize:9, color:C.muted, marginTop:2 }}>{r.date} · {r.charity}</div>
                  </div>
...
function Chat({ activities, stats, whoopData, whoopOk, onPlanSaved, corosSession, onCorosSessionHandled }) {
  const [messages, setMessages] = useState([{ role:"assistant", content:"Hi Caleb! I know everything about you and your training. Ask me anything — running, recovery, training plans, race strategy, gym, nutrition. If you want a training plan saved to your Plan tab, just ask!" }]);
  ...
  const send = async () => {
    ...
    try {
      const res = await fetch("/.netlify/functions/claude-chat", {
        ...
        body:JSON.stringify({ system:buildContext(), messages:[...messages.filter(m=>m.role!=="system"),userMsg].map(m=>({role:m.role,content:m.content})) })
      });
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if(pending) {
      ...
      exchangeWhoopCode(code).then(()=>{setWhoopOk(true);setWhoopPending(false);}).catch(e=>{console.error(e);setWhoopPending(false);}).finally(()=>window.history.replaceState({},"","/"));
    } else if(!isConnected()) {
      exchangeCode(code).then(()=>setConnected(true)).catch(console.error).finally(()=>window.history.replaceState({},"","/"));
    }
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([getAthlete(), getActivities(100)])
      .then(([a,acts]) => {
        setAthlete(a);
        setActivities(acts);
        setBestEfforts(extractBestEfforts(acts));
        return Promise.all([getStats(a.id), getAllGear(a)]);
      })
  ...
  const views = {
    overview: <Overview {...sharedProps} bestEfforts={bestEfforts} gear={gear}/>,
    ...
    plan:     <TrainingPlan onChat={()=>setPage("chat")} externalPlan={savedPlan} onAddToCoros={(s)=>{setCorosSession(s);setPage("chat");}}/>,
    ...
    <div style={{ display:"flex", height:"100vh", background:C.bg, color:C.text, fontFamily:"Inter,sans-serif", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", height:"100vh", overflow:"hidden" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 16px", borderBottom:`1px solid ${C.border}`, background:C.surface, flexShrink:0 }}>
          <button onClick={()=>setSidebarOpen(!sidebarOpen)} style={{ background:"transparent", border:`1px solid ${C.border}`, color:C.sub, borderRadius:7, padding:"5px 9px", fontSize:14, cursor:"pointer", flexShrink:0, lineHeight:1 }}>
            {sidebarOpen?"✕":"☰"}
          </button>
