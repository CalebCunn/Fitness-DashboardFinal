...
function Nutrition({ userPrefs, onSavePrefs }) {
  ...
  const todayLog = nutrition[today] || { kcal:"", carbs:"", protein:"", fat:"", notes:"" };

  const save = (field, value) => {
    const updated = { ...nutrition, [today]: { ...todayLog, [field]: value } };
    // Keep last 30 days only
    const keys = Object.keys(updated).sort().slice(-30);
    const trimmed = {};
    keys.forEach(k => trimmed[k] = updated[k]);
    onSavePrefs({ ...userPrefs, nutrition: trimmed });
  };

  const targets = { kcal: 3000, carbs: 300, protein: 140, fat: 80 };
  const history = Object.entries(nutrition).sort((a,b)=>b[0].localeCompare(a[0])).slice(0,7);

  const Field = ({ label, field, unit, color }) => {
    const val = parseFloat(todayLog[field]) || 0;
    const target = targets[field];
    const pct = target ? Math.min(100, Math.round(val/target*100)) : 0;
    return (
      <div style={{ background:C.bg, borderRadius:12, padding:"12px 16px" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          <div style={{ fontSize:11, fontWeight:600, color:C.sub, textTransform:"uppercase", letterSpacing:"0.04em", fontFamily:C.sans }}>{label}</div>
          <div style={{ fontSize:11, color:C.muted, fontFamily:C.sans }}>{target ? `${val} / ${target}${unit}` : ""}</div>
        </div>
        <input
          type="number"
          value={todayLog[field]}
          onChange={e=>save(field,e.target.value)}
          placeholder={`Target: ${target}${unit}`}
          style={{ width:"100%", background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"7px 10px", color:C.text, fontSize:14, fontFamily:C.mono, fontWeight:700, outline:"none", marginBottom:8 }}
        />
        {target > 0 && (
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:14, paddingBottom:20 }}>
      <Card>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10, marginBottom:12 }}>
          <Field label="Calories" field="kcal" unit="kcal" color={C.orange}/>
          <Field label="Carbs" field="carbs" unit="g" color={C.blue}/>
          <Field label="Protein" field="protein" unit="g" color={C.red}/>
          <Field label="Fat" field="fat" unit="g" color={C.yellow}/>
        </div>
        <div style={{ background:C.bg, borderRadius:12, padding:"12px 16px" }}>
          <div style={{ fontSize:11, fontWeight:600, color:C.sub, textTransform:"uppercase", letterSpacing:"0.04em", marginBottom:8, fontFamily:C.sans }}>Notes</div>
          <textarea
            value={todayLog.notes}
            onChange={e=>save("notes",e.target.value)}
            placeholder="What did you eat today? Any notes..."
            rows={3}
            style={{ width:"100%", background:C.surface, border:`1px solid ${C.border}`, borderRadius:8, padding:"8px 10px", color:C.text, fontSize:13, fontFamily:C.sans, outline:"none", resize:"vertical" }}
          />
        </div>
        <div style={{ marginTop:12, padding:"10px 14px", background:C.orangeL, borderRadius:10 }}>
          <div style={{ fontSize:11, color:C.orange, fontWeight:600, marginBottom:4, fontFamily:C.sans }}>Daily Targets</div>
          <div style={{ fontSize:12, color:C.sub, fontFamily:C.sans }}>2,800-3,200 kcal · 250-350g carbs · 130-150g protein</div>
        </div>
      </Card>
      {history.length > 0 && (
        <Card>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {history.filter(([d])=>d!==today).slice(0,6).map(([date,log])=>(
              <div key={date} style={{ background:C.bg, borderRadius:10, padding:"10px 14px", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:8 }}>
                <div style={{ fontSize:12, fontWeight:600, color:C.text, fontFamily:C.sans }}>{new Date(date).toLocaleDateString("en-GB",{weekday:"short",day:"numeric",month:"short"})}</div>
                <div style={{ display:"flex", gap:12 }}>
                  {log.kcal&&<div style={{ fontSize:11, color:C.orange, fontFamily:C.mono, fontWeight:600 }}>{log.kcal}kcal</div>}
                  {log.protein&&<div style={{ fontSize:11, color:C.red, fontFamily:C.mono }}>{log.protein}g pro</div>}
                  {log.carbs&&<div style={{ fontSize:11, color:C.blue, fontFamily:C.mono }}>{log.carbs}g carbs</div>}
                </div>
              </div>
            ))}
          </div>
...
// ─── CHAT ────────────────────────────────────────────────────────────────────
function Chat({ activities, stats, whoopData, whoopOk, onPlanSaved, corosSession, onCorosSessionHandled, userPrefs }) {
...
LIVE NUTRITION: Check the nutrition tab for logged macros.
...
export default function App() {
  ...
  const views={
    ...
    chat:<Chat {...sharedProps} onPlanSaved={setSavedPlan} corosSession={corosSession} onCorosSessionHandled={()=>setCorosSession(null)} userPrefs={userPrefs}/>,
  };
